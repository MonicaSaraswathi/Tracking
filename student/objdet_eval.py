    ...
    for label, valid in zip(labels, labels_valid):
        ...
        if valid: # exclude all labels from statistics which are not considered valid
            ...
            for a in detections:
                ...
                _,_x,_y,_z,_h,_w,_l,_yaw=a
                detection_corners=tools.compute_box_corners(_x,_y,_w,_l,_yaw)
                ...
                det_Label=Polygon(detection_corners)
                iou=poly_Label.intersection(det_Label).area/poly_Label.union(det_Label).area
...
def compute_performance_stats(det_performance_all):
    ...
    total,tp,fn,fp=pos_negs.sum(axis=0)
